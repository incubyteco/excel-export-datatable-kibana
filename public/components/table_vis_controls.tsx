...
export const TableVisControls = memo(({ dataGridAriaLabel, ...props }: TableVisControlsProps) => {
  ...
  const button = (
    ...
    >
      <FormattedMessage id="visTypeTable.vis.controls.exportButtonLabel-1" defaultMessage="Export..." />
    </EuiButtonEmpty>
  );

  const items = [
    <EuiContextMenuItem key="rawCsv" onClick={() => onClickExport(false)}>
      <FormattedMessage id="visTypeTable.vis.controls.rawCSVButtonLabel-1" defaultMessage="Raw..." />
    </EuiContextMenuItem>,
