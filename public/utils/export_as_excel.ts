...
export const exportAsExcel = (formatted: boolean, data: ToCsvData) => {
  const rawResponse = toCsv(formatted, data);

  const workbook = read(rawResponse, { type: 'string', raw: true });
  writeFile(workbook, `${data.filename || 'unsaved'}.xlsx`, { type: 'binary' });
};
